...
use super::movement::{open_blocking_door, MAX_STEP_DIST};
...
pub(super) fn equipped_weapon_skill(state: &SharedState) -> Option<SkillId> {
    state
        .self_equipped
        .get(&EquipSlot::MainHand)
        .and_then(|item| crate::item_defs::get(&item.item_def_id))
        .and_then(|def| def.weapon_skill)
}

pub(super) fn equipped_weapon_attack_cooldown(state: &SharedState) -> Duration {
    let cooldown_ms = equipped_weapon_skill(state)
        .map(weapon_skill_attack_cooldown_ms)
        .unwrap_or(DEFAULT_WEAPON_ATTACK_COOLDOWN_MS);
    Duration::from_millis(u64::from(cooldown_ms))
}

fn equipped_weapon_melee_range(state: &SharedState) -> f32 {
    equipped_weapon_skill(state)
...
impl ChaseTarget<'_> {
    ...
    fn tuning(&self) -> ChaseTuning {
        match self {
            ...
            Self::Monster { arrive_range, .. } => ChaseTuning {
...
pub(super) async fn chase_monster(
    ...
    let arrive_range = {
        ...
        equipped_weapon_melee_range(&state)
    };
    chase_target(
        state,
        &ChaseTarget::Monster {
...
async fn chase_target(state: &Arc<Mutex<SharedState>>, target: &ChaseTarget<'_>) -> ChaseResult {
    ...
    loop {
        ...
        // walks at the server-authored effective speed. Fall back to a direct
        // step toward the target
        // if A* returned no path.
        let (step_dist, movement_speed) = if path_index < path_waypoints.len() {
            ...
            if to_wp.dist < 0.1 {
                path_index += 1;
                (0.0, s.effective_movement_speed())
            } else {
            ...
            match compute_step_toward(&s, target) {
                ...
                None => (0.0, s.effective_movement_speed()),
            }
        };

        let travel_ms = if step_dist > 0.0 {
            ((step_dist / movement_speed) * 1000.0) as u64
        } else {
            CHASE_IDLE_TICK_MS
        };
