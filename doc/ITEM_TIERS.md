...
- 골드는 `깊이 × 500 ~ 깊이 × 1500`.
- **보스가 쓰러질 때 20m 안에 있던 캐릭터만** 열 수 있다(`CHEST_CLAIM_RADIUS`). 보스는 던전 리셋(일몰, 실제 3시간)에만 되살아나므로 한 밤에 한 번뿐이다. 보스 처치 여부와 이 권한은 메모리에만 있어 **서버 재시작은 리셋과 같이 취급된다** — 보스가 되살아나고 그날 밤의 권한은 사라진다.
- 캐릭터당 하룻밤(게임 시간) 1회. 실제로 열면 60초 뒤 마을로 귀환시킨다(`CHEST_RETURN_DELAY`; 그 사이 로그아웃하면 즉시 귀환한 위치로 저장) — 최심층에 캐릭터를 세워두고 리필마다 로그인만 하던 파킹을 막는다. 빈 상자 재클릭은 귀환을 걸지 않는다.
...
| 2 | iron_sword | 1d8 | 10,000 | orc_boss 무장·확정 드랍, 월드 드랍 0.5% (상점 비판매) | 있음 |
