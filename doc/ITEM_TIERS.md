...
세트 guard 합계 = 6. Leather Armor chest는 추가로 slash·pierce·blunt
...
| boots | plate_boots | 3 | 3 | 있음 (티어 3 던전 생기기 전까지 미드랍) |
...
효과 장신구는 NetHack 계열로 간다 (효과 수치는 구현 시 확정). 무효과 장신구 2종(silver_necklace, gold_ring)은 chestTier 미지정으로 체스트 풀에서 빠진다 — 현재 신규 획득 경로 없음(구 체스트 규칙 유통분만 잔존, 다수가 착용 중). 치장 수요가 확인되어 추후 상점 사치품(성능 0, 골드 싱크)化 검토.

| 아이템 | 효과 | 획득처 | 상태 |
|--------|------|--------|------|
| ring_of_protection | guard +1 | 티어 5 던전 (20% 롤) | 있음 |
...
- 던전당 시그니처(확정) 드랍 1개: **각 세트의 몸통(핵심) 파츠가 완성 티어의 시그니처** — Old Crypt는 leather_helmet, Orc Warrens는 leather_armor, 티어 3은 chain_mail, 티어 4는 breastplate. 티어 5는 시그니처 없음 (확정이면 1회 만에 끝나므로 ring_of_protection은 20% 롤).
...
- **보스: 정품 확정 드랍** (티어 5만 20% 롤) — 판매 경제는 밤당 1자루로 캡.
