...
- 시계 아이템 (현재 시간을 알려줌. Debug 용으로 있는 CAL 기능을 켜면 될 듯)
- 물어 젖으면 무거워지게. 이속 저하
- 채팅 창 위치 변경
- [x] 레벨과 경험치 늘 표시
...
- [x] 방패가 세워져 있음
- 월드 드랍 혹은 저렙 몬스터 먹을 것 드롭하기
- 아이템 살 때 최대 한도가 아니라 적당한 묶음으로
- 체스트가 겹쳐서 나오는 경우가 있음 Position: world(-1768.8, -2.6, 5039.2) tile(-28, 79) cell(55, 15)
- 나무 통 부서지는 소리 약간 작게
- 체스트에서 나온 금화가 계단 밑으로 떨어져서 못 주움 Position: world(-1751.8, -2.6, 5100.6) tile(-27, 80) cell(8, 12) 주울 순 있는데 이상함. 체스트를 계단 틈에 배치하지 않게
- Enchant weapon 주문서가 너무 잘 나옴
- 코볼트 죽는 애니메이션이 끝나고 나서 땅에 붙음
- 경험치 얻을 때 경험치 게이지에 하이라이트가 한바퀴 돌게
- 아이템 주울 때 캐릭터 방향 회전
- 무게도 게이지로
- Position: world(-1757.2, -26.6, 5064.6) tile(-27, 79) cell(2, 40) 이 위치에 엄청나게 넓은 문이 있음 Position: world(-1758.3, -54.6, 5083.5) tile(-27, 79) cell(1, 59) 여기도
- 스토커 죽는 애니 이상함
- 몬스터가 죽을 때 나는 소리
- 시간 위짓에 던전 층 수 표시
- 더 큰 회복 물약
- 숨겨진 계단
- 가드 36인데 마지막층에서 거의 무적
- bag of holdings
- 마을로 되돌려 보내는 1분 좀 짧음
