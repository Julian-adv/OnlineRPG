...
#[test]
fn stack_into_bag_merges_only_same_def_enchant_and_condition() {
    let mut bag = Vec::new();
    stack_into_bag(&mut bag, true, "apple", 0, 1, 1, None);
    stack_into_bag(&mut bag, true, "apple", 0, 2, 1, None);
    stack_into_bag(&mut bag, true, "apple", 1, 3, 1, None);
    stack_into_bag(&mut bag, false, "torch", 0, 4, 1, None);
    stack_into_bag(&mut bag, false, "torch", 0, 5, 1, None);
    stack_into_bag(&mut bag, true, "apple", 0, 6, 1, Some(3));
    stack_into_bag(&mut bag, true, "apple", 0, 7, 1, Some(4));

    assert_eq!(bag.len(), 6, "different condition values stay separate");
