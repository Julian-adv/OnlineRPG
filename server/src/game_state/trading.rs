...
impl super::GameState {
    ...
    pub async fn buy_item(
        ...
        let (snapshot, npc_snapshot) = {
            ...
            stack_into_bag(
                &mut inv.bag,
                stackable,
                &transferred_item.item_def_id,
                transferred_item.enchant,
                transferred_item.instance_id,
                transferred_item.quantity,
                transferred_item.durability,
            );
...

        {
            ...
            if is_resident {
                for transferred in transferred_items {
                    let stackable = self
                        .item_defs
                        .get(&transferred.item_def_id)
                        .is_some_and(|d| d.stackable);
                    let units_per_insert = if stackable { transferred.quantity } else { 1 };
                    let inserts = if stackable { 1 } else { transferred.quantity };
                    for _ in 0..inserts {
                        let instance_id = self.next_instance_id().await;
                        stack_into_bag(
                            &mut inv.bag,
                            stackable,
                            &transferred.item_def_id,
                            transferred.enchant,
                            instance_id,
                            units_per_insert,
                            transferred.durability,
                        );
                    }
                }
            } else {
                for plan in &plans {
                    let stackable = self
                        ...
                        .is_some_and(|d| d.stackable);
                    let units_per_insert = if stackable { plan.qty } else { 1 };
                    let inserts = if stackable { 1 } else { plan.qty };
                    for _ in 0..inserts {
                        let instance_id = self.next_instance_id().await;
                        stack_into_bag(
                            &mut inv.bag,
                            ...
                            instance_id,
                            units_per_insert,
                            self.initial_item_durability(&plan.item_def_id),
                        );
                    }
                }
            }
    ...
    pub async fn sell_items(
        ...
        struct Plan {
            ...
            condition_percent: u32,
            base_price: i64,
            deal_taken: Option<DealEntry>,
            deal_unit_payout: Option<i64>,
            normal_unit_payout: i64,
            payout: i64,
        }
        ...
        for req in &items {
            ...
            let Some((base_price, max_durability)) = self
                .item_defs
                .get(&item_def_id)
                .and_then(|d| d.base_price.map(|price| (price, d.max_durability)))
            else {
                drop(inventories);
                drop(gold_map);
                return self
            ...
            let condition_percent = durability
                .zip(max_durability)
                .and_then(|(current, max)| durability_value_percent(current, max))
                .unwrap_or(100);
            plans.push(Plan {
                ...
                base_price,
                deal_taken: None,
                deal_unit_payout: None,
                normal_unit_payout: 0,
                payout: 0,
            });
        }
        if plans.is_empty() {
        ...
        for plan in &mut plans {
            ...
            let deal_units: u32 = if deal.is_some() { 1 } else { 0 };
            let normal_units = plan.qty - deal_units;
            let condition_adjusted =
                |payout: i64| (payout * i64::from(plan.condition_percent) / 100).max(1);
            let normal_unit_payout = condition_adjusted(sell_payout(plan.base_price, rate, 0));
            let deal_unit_payout = deal.as_ref().map(|entry| {
                condition_adjusted(sell_payout(plan.base_price, rate, entry.modifier_pct))
            });
            let payout = deal_unit_payout.unwrap_or(normal_unit_payout) * deal_units as i64
                + normal_unit_payout * normal_units as i64;
            plan.deal_taken = deal;
            plan.deal_unit_payout = deal_unit_payout;
            plan.normal_unit_payout = normal_unit_payout;
            plan.payout = payout;
            total_payout += payout;
        }
        ...
        *gold_map.get_mut(player_id).expect("checked above") += total_payout;
        ...
        if !is_resident {
            for plan in &plans {
                for unit in 0..plan.qty {
                    let unit_price = if unit == 0 {
                        plan.deal_unit_payout.unwrap_or(plan.normal_unit_payout)
                    } else {
                        plan.normal_unit_payout
                    };
